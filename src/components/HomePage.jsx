...
export default function HomePage({
  ...
  const features = [
    { icon: "🤖", label: "AI Salah", action: () => onOpenChat("") },
    { icon: "📊", label: "Mandi Live", action: onOpenMandi, badge: true },
    { icon: "📋", label: "Khata", action: onOpenKhata },
    { icon: "🏛️", label: "Yojna", action: onOpenYojna, badge: true },
    { icon: "🎯", label: "Quiz", action: onOpenQuiz },
    { icon: "🌧️", label: "Mausam Alert", action: onOpenWeather },
    { icon: "💬", label: "AI Chat", action: () => onOpenChat("") },
  ];
  ...
  const S = {
    ...
    cropCard: { background: "rgba(30,180,100,0.1)", border: "1px solid rgba(30,180,100,0.22)", borderRadius: 13, padding: "9px 11px", cursor: "pointer" },
    ...
    mc: { flex: 1, background: "rgba(100,180,255,0.07)", border: "1px solid rgba(100,180,255,0.13)", borderRadius: 9, padding: "6px 7px" },
  ...
  return (
    <div style={S.wrap}>
      <div onClick={onOpenBg} style={{ position: "absolute", inset: 0, zIndex: 0 }} />

      {/* Topbar */}
      <div style={S.topbar}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#60b8ff" }}>❄️ Hanuman Khad Bhandar</div>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", cursor: "pointer" }} onClick={onOpenProfile}>👤 {kisanNaam} ji</div>
        </div>
      ...
      <div style={S.scroll}>
        {/* Weather Card */}
        <motion.div style={S.wcard} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} onClick={onOpenWeather}>
          {weather ? (<>
        ...
        {/* Alert */}
        {alert && <motion.div style={S.alertBox} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>🚨 {alert}</motion.div>}

        {/* Crop Progress */}
        <motion.div style={S.cropCard} initial={{ opacity: 0 }} animate={{ opacity: 1 }} onClick={onOpenBg}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            {stages.map((s, i) => (
              <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
                <div style={{ width: 9, height: 9, borderRadius: "50%", background: i < activeStageIdx ? "#1eb464" : i === activeStageIdx ? "#60b8ff" : "rgba(255,255,255,0.1)", border: i === activeStageIdx ? "2px solid #fff" : "1px solid rgba(255,255,255,0.15)", boxShadow: i === activeStageIdx ? "0 0 5px #60b8ff88" : "none" }} />
        ...
        <div style={S.grid}>
          {features.map((f, i) => (
            <motion.div key={i} style={S.gi} whileTap={{ scale: 0.93 }} onClick={f.action}
        ...
        {/* Streak */}
        <motion.div style={S.streakCard} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          <div style={{ display: "flex", gap: 5 }}>
            {streakDays.map((d, i) => (
              <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
                <div style={{ width: 22, height: 22, borderRadius: "50%", background: d.isToday ? "#ff6600" : d.done ? "rgba(255,120,0,0.35)" : "rgba(255,255,255,0.07)", border: d.done ? "1px solid #ff9944" : "1px solid rgba(255,255,255,0.12)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10 }}>
                  {d.done ? (d.isToday ? "🔥" : "✓") : ""}
                </div>
        ...
        {mandiData.length > 0 && (
          <div style={{ display: "flex", gap: 5 }}>
            {mandiData.map((m, i) => (
              <motion.div key={i} style={S.mc} whileTap={{ scale: 0.95 }} onClick={onOpenMandi}>
        ...
        {mandiLoading && <div style={{ textAlign: "center", fontSize: 11, color: "rgba(100,180,255,0.4)" }}>📊 Mandi bhav dhundh raha hoon...</div>}

        {/* Leaderboard */}
        <motion.div style={S.lb} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          {[...leaderboard, { naam: kisanNaam + " (Tu)", points, isYou: true }]
            ...
            .map((l, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 5px", borderRadius: 6, background: l.isYou ? "rgba(100,180,255,0.1)" : "transparent", marginBottom: 2 }}>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", width: 14 }}>{i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `${i+1}.`}</div>
      ...
      <div style={S.bottomnav}>
        {[
          { icon: "🏠", label: "Home", key: "home", action: () => setActiveNav("home") },
          { icon: "🌤️", label: "Weather", key: "weather", action: onOpenWeather },
          { icon: "💬", label: "AI", key: "ai", action: () => onOpenChat("") },
          { icon: "📊", label: "Mandi", key: "mandi", action: onOpenMandi },
          { icon: "👤", label: "Profile", key: "profile", action: onOpenProfile },
        ].map(n => (
          <div key={n.key} onClick={() => { setActiveNav(n.key); n.action(); }}
            style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, fontSize: 9, color: activeNav === n.key ? "#60b8ff" : "rgba(255,255,255,0.28)", cursor: "pointer" }}>
